...
void setup() {
  ...
  for (int i = 0; i < MAX_NUM_BLOCKS; i++) {
    BLOCK_POSITIONS[i] = new PVector(random(-H_A_S, H_A_S), 
                                     random(-H_A_S, H_A_S), 
                                     random(-H_A_S, H_A_S));
    BLOCK_COLORS[i] = color(random(255), random(255), random(255));
  }
...
void draw() {
  ...
  if (USING_SERIAL)  handle_controls();
  handle_keys();
  ...
  if (P_DOWN)                        // maybe just go to console
    render_data();

  // iterate the content
  if (LIFE_PERIOD.is_zero())  LIFE.iterate();
...
//  float s = s_n - s_o;
  ...
  NUM_BLOCKS = int(map(l, 0, 1023, MAX_NUM_BLOCKS, 0));
}
